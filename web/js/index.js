...
function showHotels(listHotels) {
    ...
    if (listHotels.length < 1) {
        ...
        $.each(listHotels, function (i, hotel) {
            ...
            rowContent += "<button class='btn btn-primary btn-material px-5 py-2 btn-add-cart' onclick=\"addToCart('" + hotel.hotelId + "','" + hotel.typeId + "','" + hotel.checkIn + "','" + hotel.checkOut + "','"+ hotel.quantity +"')\">Add to Cart</button>";
            rowContent += "</div>";
            rowContent += "</div>";
            rowContent += "</div>";
            rowContent += "</div>";
...
// Function add hotel to cart
function addToCart(hotelId, typeId, checkIn, checkOut, quantity) {
    $.ajax({
        url: "addToCart",
        type: "POST",
        data: {
